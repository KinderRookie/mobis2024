...
def socket_server():
    ...
    metric_x_left = round(point_on_plane_left[0] * 100 / 2.54 * (-1))
    metric_z_left = round(point_on_plane_left[2] * 100 / 2.54 *(-1))
    metric_x_right = round(point_on_plane_right[0] * 100 / 2.54 * (-1))
    metric_z_right = round(point_on_plane_right[2] * 100 / 2.54 *(-1))
...
def cal_Point(image_width, image_height, ref_width, ref_distance, img_x, img_y, theta_x, theta_y):
    ...
    return Point(rotated_x, rotated_y-0.1, rotated_z-0.14)
...
while True:
    ...
    if left_eye == (0, 0) and right_eye == (0, 0):
        defalut_mode()
        continue
    left_eye_point = cal_Point(IMAGE_WIDTH, IMAGE_HEIGHT, REF_WIDTH, REF_DISTANCE, left_eye[0], left_eye[1],0.0536,-0.0079)
    right_eye_point = cal_Point(IMAGE_WIDTH, IMAGE_HEIGHT, REF_WIDTH, REF_DISTANCE, right_eye[0], right_eye[1],0.0536,-0.0079)
